    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          const Text("Join the premiere cinema club",
          ...
          Row(children: [
            Expanded(child: RadioListTile<String>(
                title: const Text("Customer"), value: 'Customer',
                groupValue: _role,
                onChanged: (v) => setState(() => _role = v!),
                activeColor: Colors.amber, contentPadding: EdgeInsets.zero)),
            Expanded(child: RadioListTile<String>(
                title: const Text("Admin"), value: 'Admin',
                groupValue: _role,
                onChanged: (v) => setState(() => _role = v!),
                activeColor: Colors.amber, contentPadding: EdgeInsets.zero)),
          ]),
...
class ReviewOrderPage extends StatelessWidget {
            ...
            child: Row(children: [
              GestureDetector(
                  ...
                  child: const Icon(Icons.arrow_back_ios_new,
                      color: TC.textPri, size: 18)),
              const Expanded(
                child: Text('REVIEW ORDER',
                    textAlign: TextAlign.center,
                    style: TextStyle(color: TC.textPri, fontSize: 16,
                        fontWeight: FontWeight.w700, letterSpacing: 1)),
              ),
              const Icon(Icons.person_outline, color: TC.textPri, size: 22),
            ]),
          ),
          Expanded(
            child: SingleChildScrollView(
                ...
                ClipRRect(
                  ...
                  child: Image.network(movie.imgUrl,
                      ...
                      errorBuilder: (_, __, ___) => Container(
                          height: 200, color: TC.surface,
                          child: Center(child: Text(movie.title[0],
                              style: const TextStyle(color: TC.accent, fontSize: 64,
                                  fontWeight: FontWeight.w900))))),
                ),
                const SizedBox(height: 20),
                Text(movie.title,
                    textAlign: TextAlign.center,
                    style: const TextStyle(color: TC.accent, fontSize: 22,
                        fontWeight: FontWeight.w900, letterSpacing: 0.8)),
                const SizedBox(height: 6),
                const Text('OCT 24  •  20:30  •  HALL 01',
                    style: TextStyle(color: TC.textSec, fontSize: 12)),
                const SizedBox(height: 24),
                // Order card
                Container(
                  padding: const EdgeInsets.all(20),
                  ...
                  child: Column(children: [
                    _row(icon: Icons.chair_outlined, label: 'Selected seats',
                        value: _seatLabel, valueColor: TC.accent, bold: true),
                    Divider(color: TC.surface, height: 28),
                    _row(label: 'Ticket price',
                        value: '\$${ticketPrice.toStringAsFixed(2)} × ${selectedSeats.length}'),
                    const SizedBox(height: 10),
                    _row(label: 'Convenience fee',
                        value: '\$${convFee.toStringAsFixed(2)}'),
                    Divider(color: TC.surface, height: 28),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      const Text('GRAND TOTAL',
                          style: TextStyle(color: TC.textSec, fontSize: 11,
                              letterSpacing: 1.2, fontWeight: FontWeight.w700)),
                      Text('\$${_total.toStringAsFixed(2)}',
                          style: const TextStyle(color: TC.accent, fontSize: 28,
                              fontWeight: FontWeight.w900)),
                    ]),
                  ]),
                ),
  ...
  Widget _row({IconData? icon, required String label, required String value,
      Color? valueColor, bool bold = false}) {
    return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Row(children: [
        if (icon != null) ...[
          Icon(icon, color: TC.textSec, size: 15),
          const SizedBox(width: 6),
        ],
...
class _PaymentPageState extends State<PaymentPage> {
  ...
  double get _tax => widget.subtotal * 0.10;
  static const _svcFee = 0.50;
  double get _grandTotal => widget.subtotal + _tax + _svcFee;
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: TC.card,
                      borderRadius: BorderRadius.circular(16),
                      border: Border.all(color: TC.accent.withOpacity(0.25))),
                  child: Row(children: [
                    ClipRRect(
                      borderRadius: BorderRadius.circular(8),
                      child: Image.network(widget.movie.imgUrl,
                          width: 56, height: 70, fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) => Container(
                              width: 56, height: 70, color: TC.surface,
                              child: Center(child: Text(widget.movie.title[0],
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(widget.movie.title,
                          style: const TextStyle(color: TC.textPri, fontSize: 13,
                              fontWeight: FontWeight.w700),
                          maxLines: 1, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 4),
                      Row(children: [
                        const Icon(Icons.access_time, color: TC.textSec, size: 11),
                        const SizedBox(width: 4),
                        const Text('20:30', style: TextStyle(color: TC.textSec, fontSize: 11)),
                        const SizedBox(width: 8),
                        const Icon(Icons.chair_outlined, color: TC.textSec, size: 11),
                        const SizedBox(width: 4),
                        Flexible(child: Text(_seatLabel,
                            style: const TextStyle(color: TC.textSec, fontSize: 11),
                            overflow: TextOverflow.ellipsis)),
                      ]),
                      const SizedBox(height: 8),
                      Text('\$${widget.subtotal.toStringAsFixed(2)}',
                          style: const TextStyle(color: TC.accent, fontSize: 18,
                              fontWeight: FontWeight.w900)),
                    ])),
                    const SizedBox(width: 8),
                    Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          border: Border.all(color: TC.accent.withOpacity(0.35))),
                      child: const Text('PREMIUM\nSCREEN',
                          textAlign: TextAlign.center,
                          style: TextStyle(color: TC.accent, fontSize: 8,
                ...
                _payOpt(id: 'cash', icon: Icons.payments_outlined,
                    title: 'Cash (Tunai)', sub: 'Pay at the counter or arrival'),
                const SizedBox(height: 10),
                _payOpt(id: 'ewallet', icon: Icons.account_balance_wallet_outlined,
                    title: 'E-Wallet', sub: 'Fast & Secure payment.'),
                const SizedBox(height: 28),
                // Fee breakdown
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      border: Border.all(color: TC.surface)),
                  child: Column(children: [
                    _feeRow('Subtotal', '\$${widget.subtotal.toStringAsFixed(2)}'),
                    const SizedBox(height: 10),
                    _feeRow('Tax (10%)', '\$${_tax.toStringAsFixed(2)}'),
                    const SizedBox(height: 10),
                    _feeRow('Service Fee', '\$${_svcFee.toStringAsFixed(2)}'),
                    Divider(color: TC.surface, height: 20),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      const Text('TOTAL',
                          style: TextStyle(color: TC.textSec, fontSize: 11,
                              letterSpacing: 1.2, fontWeight: FontWeight.w700)),
